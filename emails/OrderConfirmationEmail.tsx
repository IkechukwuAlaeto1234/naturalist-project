      ...
      <div style={{ fontFamily: "system-ui, -apple-system, sans-serif", color: "#141f19" }}>
        ...
        <table width="100%" cellPadding={0} cellSpacing={0} style={{ border: "1px solid #e2dacd", borderRadius: "16px", overflow: "hidden", marginBottom: "24px", width: "100%", borderCollapse: "separate" }}>
          ...
          <tbody>
            {items.map((item, idx) => (
              <tr key={idx}>
                <td style={{ padding: "14px 18px", fontSize: "14px", color: "#141f19", borderBottom: "1px solid #f4efe6" }}>{item.name}</td>
                <td align="center" style={{ padding: "14px 18px", fontSize: "14px", color: "#5e6f64", borderBottom: "1px solid #f4efe6" }}>{item.quantity}</td>
                <td align="right" style={{ padding: "14px 18px", fontSize: "14px", color: "#141f19", borderBottom: "1px solid #f4efe6", fontWeight: "bold" }}>
                  ${(item.price * item.quantity).toFixed(2)}
                </td>
              </tr>
            ))}
            <tr>
              <td colSpan={2} align="right" style={{ padding: "18px 18px 18px 18px", fontWeight: "bold", fontSize: "13px", color: "#5e6f64", textTransform: "uppercase", letterSpacing: "1px" }}>
                Total Paid:
              </td>
              <td align="right" style={{ padding: "18px 18px 18px 18px", fontWeight: "900", fontSize: "18px", color: "#2d4c38" }}>
