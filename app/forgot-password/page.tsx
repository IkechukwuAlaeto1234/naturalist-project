...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmed || !/^\S+@\S+\.\S+$/.test(trimmed)) {
      ...
      window.location.href = "/forgot-password/lookup?status=not_found";
      return;
    }
    ...
    try {
      ...
      // Enforce 1.2 s minimum so the spinner feels intentional
      const elapsed = Date.now() - startTime;
      await new Promise((r) => setTimeout(r, Math.max(1200 - elapsed, 0)));
      ...
      if (res.ok && data?.success && data?.token) {
        ...
        window.location.href = `/forgot-password/lookup?ref=${encodeURIComponent(data.token)}`;
      } else {
