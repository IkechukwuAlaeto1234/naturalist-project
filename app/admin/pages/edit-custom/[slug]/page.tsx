...
export default function EditCustomPagePage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const savedSuccess = sessionStorage.getItem("custom_page_save_success");
      if (savedSuccess) {
        setSuccessModalOpen(true);
        sessionStorage.removeItem("custom_page_save_success");
      }
    }
  }, []);
  ...
  useEffect(() => {
    if (!slugParam) return;
    setLoading(true);
    fetch(`/api/admin/content?key=page-${slugParam}`, { cache: "no-store" })
      ...
      .then((data) => {
        if (data) {
          ...
          setError("Custom page data not found.");
        }
      })
  ...
  return (
    <div className="space-y-8 pb-20 max-w-5xl mx-auto">
      ...
      <div>
        ...
        <h1 className="font-serif text-3xl font-bold tracking-tight text-white mt-1">Edit Custom Page</h1>
        <p className="text-xs text-[#a3b2a9] mt-1">Your page is accessible at <span className="text-[#b07e3a]">/p/{slug}</span></p>
      </div>
      ...
      {saved && (
        <div className="flex items-center gap-2.5 px-4 py-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5 text-emerald-400 text-xs font-semibold">
          <CheckCircle2 className="h-4 w-4" /> Page updated successfully! Redirecting…
        </div>
      )}
      {error && (
      ...
      <form onSubmit={handleSave} className="flex flex-col gap-5">
        ...
        <button type="submit" disabled={saving || !title || !slug} className="flex items-center justify-center gap-2 w-full py-3.5 rounded-2xl bg-[#b07e3a] hover:bg-[#9a6e30] text-white text-xs font-bold uppercase tracking-wider transition-all disabled:opacity-60">
          ...
          {saving ? "Saving Changes…" : "Save Page Changes"}
        </button>
      </form>
      ...
      <SuccessModal
        ...
        title="Custom Page Saved"
        message="Your custom page content has been reloaded from the database, and the changes are now live!"
        actionText="Close"
        showCancel={false}
        showClose={true}
