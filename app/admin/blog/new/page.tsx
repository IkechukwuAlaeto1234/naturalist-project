...
export default function AdminNewBlogPage() {
  ...
  const [formAuthorName, setFormAuthorName] = useState("Alaeto Ikechukwu Miracle");
  const [formAuthorRole, setFormAuthorRole] = useState("Founder & Ritual Specialist");
  const [formReadTime, setFormReadTime] = useState("4 min read");
  const [formTags, setFormTags] = useState("skincare, rituals, wellness");
  ...
  useEffect(() => {
    document.title = "Compose Skincare Article | Naturalist";
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast("success", "Article Published", `${formTitle} is now live.`);
    ...
    <div className="space-y-8 pb-20 font-sans text-white max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="flex items-center gap-1 text-[9px] font-black uppercase tracking-wider bg-[#b07e3a]/10 text-[#b07e3a] border border-[#b07e3a]/20 px-3 py-1 rounded-full">
          <Sparkles className="h-3 w-3" /> Editorial Compose Desk
        </span>
      </div>

      {/* ── Page Title ── */}
      <div>
        <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-[#b07e3a]">Editorial Desk</span>
        <h1 className="font-serif text-5xl font-black tracking-tight leading-none mt-1">Compose Skincare Article</h1>
        <p className="text-xs text-[#a3b2a9] mt-2">Write a premium story, holistic spotlight, or formulation study for the Naturalist blog.</p>
      </div>
