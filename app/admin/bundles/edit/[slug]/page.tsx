...
export default function AdminEditBundlePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [bundleRes, productsRes] = await Promise.all([
        fetch(`/api/bundles/${slug}`, { cache: "no-store" }),
        fetch("/api/products?includeInactive=true", { cache: "no-store" })
      ]);

      if (!bundleRes.ok) throw new Error("Ritual bundle not found.");
      if (!productsRes.ok) throw new Error("Failed to load products list.");

      const bundleData: BundleItem = await bundleRes.json();
      const productsData: Product[] = await productsRes.json();

      setProductsList(productsData);
      setName(bundleData.name);
      setDescription(bundleData.description || "");
      setPrice(String(bundleData.price));
      setComparePrice(bundleData.compareAtPrice ? String(bundleData.compareAtPrice) : "");
      setImage(bundleData.images?.[0] || "");
      
      const prodIds = bundleData.products.map((p) => typeof p === "string" ? p : p._id || p);
      setSelectedProducts(prodIds.filter(Boolean));
      setIsActive(bundleData.isActive);
      setIsFeatured(bundleData.isFeatured || false);

      document.title = `Edit Bundle: ${bundleData.name} | Naturalist`;
    } catch (e: any) {
      setError(e.message || "Failed to load bundle details.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showToast("error", "Upload Failed", err.message || "Upload failed.");
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name || !description || !price || !image || selectedProducts.length === 0) {
      showToast("error", "Update Failed", "Please complete all required fields and select at least one product.");
      return;
    }
    ...
    try {
      ...
      showToast("success", "Bundle Updated", `Bundle overrides for ${name} saved successfully.`);
      router.push("/admin/bundles");
      router.refresh();
    } catch (err: any) {
      showToast("error", "Save Failed", err.message || "An error occurred.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center gap-3 text-white">
        ...
        <p className="text-xs text-[#a3b2a9] tracking-wider uppercase font-serif">Compiling specifications...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-destructive/20 bg-destructive/5 p-6 flex items-start gap-4 text-white">
        ...
        <div>
          <p className="text-sm font-bold text-destructive">Bundle failed to load</p>
          <p className="text-xs text-[#a3b2a9] mt-1">{error}</p>
          <a href="/admin/bundles" className="mt-4 inline-flex items-center gap-1.5 text-xs font-bold text-[#b07e3a] hover:underline uppercase tracking-wider">
    ...
    <div className="space-y-8 pb-20 text-white max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <h1 className="font-serif text-3xl font-bold tracking-tight mt-1">Edit Ritual Bundle</h1>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 text-xs">
          ...
          <div className="grid grid-cols-2 gap-4 pt-4 border-t border-[#1a241e]">
            ...
            >
              {saving ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin font-semibold text-white" /> Saving...
                </>
              ) : (
                "Save Bundle Overrides"
              )}
            </button>
          </div>
        </form>
