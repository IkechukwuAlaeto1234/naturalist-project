...
export default function BlogIndex({ initialPosts = [], pageContent = {} }: BlogIndexProps) {
  ...
  // Pick up ?tag= query param on the client side only
  useEffect(() => {
  ...
  const postsToFilter = initialPosts || [];
  const filteredPosts = selectedTag
    ? postsToFilter.filter((post) => post.tags?.includes(selectedTag))
    : postsToFilter;
      ...
      <section className="px-6 sm:px-8 py-20 bg-white dark:bg-[#0a0d0b] transition-colors duration-300">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-12 flex justify-center">
            <Link
              href="/"
              className="inline-flex items-center gap-2 rounded-full border border-border/60 px-5 py-3 text-xs font-bold uppercase tracking-widest text-foreground transition-colors hover:bg-muted"
            >
