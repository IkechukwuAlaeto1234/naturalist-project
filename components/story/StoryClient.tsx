...
export default function StoryClient({ pageContent }: StoryClientProps) {
  ...
  // Use CMS milestones/values if they exist and have content, otherwise fall back to defaults
  const milestones = pageContent.milestones?.length > 0 ? pageContent.milestones : DEFAULT_MILESTONES;
  const values = pageContent.values?.length > 0 ? pageContent.values : DEFAULT_VALUES;
        ...
        <section className="relative w-full bg-[#111a14] py-20 px-6 sm:px-8 overflow-hidden">
          ...
          <div className="mx-auto max-w-2xl text-center flex flex-col items-center gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-2">
              <a href="/shop" className="flex h-12 items-center justify-center gap-2 rounded-full bg-[#2d4c38] hover:bg-[#3a6349] px-8 text-xs font-bold uppercase tracking-wider text-white transition-all shadow-md">
                Shop All Rituals <ArrowRight className="h-4 w-4" />
              </a>
              <a href="/sustainability" className="flex h-12 items-center justify-center rounded-full border border-white/20 hover:border-white/40 px-8 text-xs font-bold uppercase tracking-wider text-white/70 hover:text-white transition-all">
